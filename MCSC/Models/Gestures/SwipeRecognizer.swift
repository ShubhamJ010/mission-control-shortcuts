    ...
    func processFrame(_ touches: [TouchPoint], timestamp: Double) -> GestureResult? {
        ...
        case let .tracking(f1ID, f2ID, startMidY, startMidX, startedMoving, startTime):
            ...
            if abs(deltaY) >= config.swipeThreshold {
                ...
                // - Moving fingers UP towards the screen: currentMidY > startMidY -> deltaY > 0 -> Swipe Up (.swipeUp /
                // .cmdSwipeUp)
                // - Moving fingers DOWN towards the user: currentMidY < startMidY -> deltaY < 0 -> Swipe Down
                // (.swipeDown / .cmdSwipeDown)
